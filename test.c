...
result * testCuckooHashingPerformance(double epsilon, elt_t* elements, int numElements){
...
result * testLinearProbingPerformance(double epsilon, elt_t* elements, int numElements) {
  result * res = malloc(sizeof(result));

  clock_t begin = clock();
  probeArray* arr = probeArray_make((int)((1.0 + epsilon) * numElements), elements, numElements);
  clock_t end = clock();
  double time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
...
void testParameters(){
  ...
  const double epsilonValues[lengthEpsilon] = {0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0};
  const int numElementsValues[lengthElements] = {1e2}; //, 3e2, 1e3, 3e3, 1e4, 3e4, 1e5};//, 3e5, 1e6, 3e6, 1e7};
  ...
  for(int i = 0; i < lengthEpsilon; i++){
    ...
    for(int j = 0; j < lengthElements; j++){
      ...
      for(int k = 0; k < numTrials; k++) {
        ...
        result* res1 = testLinearProbingPerformance(epsilon, elements, numElements);
        result* res2 = testCuckooHashingPerformance(epsilon, elements, numElements);
