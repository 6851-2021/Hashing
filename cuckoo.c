 ...
 *                   to get kicked to (in the first array), which allows for
 * loop detection, when we're trying to kick to a reserved spot.
 * @param isOccupied2 is boolean array. Same as above, but for the second array
 * @return true if insert is successful and false if a loop in the cuckoo kick
 * occur.
 */
bool cuckooArray_kick(cuckooArray* arr, int elementIndex, bool isFirstArray,
                      bool* isOccupied1, bool* isOccupied2) {
  arr->collisions++;
  ...
  int destinationIndex =
      tabulationHash(destinationHashTable, element) % arr->size;

  if (dest[destinationIndex].exist && !destOccupied[destinationIndex]) {
    destOccupied[destinationIndex] = true;
    if (!cuckooArray_kick(arr, destinationIndex, !isFirstArray, isOccupied1,
                          isOccupied2)) {
      return false;
    }
 ...
 * @param isOccupied1 is boolean array initialized to all false. Used to reserve
 * spots for elements to get kicked to (in the first array), which allows for
 * loop detection, when we're trying to kick to a reserved spot.
 * @param isOccupied2 is boolean array initialized to all false. Used to reserve
 * spots for elements to get kicked to (in the first array), which allows for
 * loop detection, when we're trying to kick to a reserved spot.
 * @return true if insert is successful and false if a loop in the cuckoo kick
 * occur.
 */
bool cuckooArray_insert(cuckooArray* arr, elt_t element, bool* isOccupied1,
                        bool* isOccupied2) {
 ...
 * @return true if insert is successful and false if a loop in a cuckoo kick
 * occur.
 */
...
bool cuckooArray_query(cuckooArray* arr, elt_t element) {
  elt_t possible_element_1 =
      arr->start1[tabulationHash(arr->table1, element) % arr->size].key;
  elt_t possible_element_2 =
      arr->start2[tabulationHash(arr->table2, element) % arr->size].key;
...
/*
 * Initialize cuckoo array hash table with a specific table size and elements
 * inserted into it
 * @param tableSize is size of hash table
